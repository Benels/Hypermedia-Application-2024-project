      ...
      <div class="flex flex-col gap-2 px-2 w-full sm:max-w-[648px] mx-auto">
        ...
        <Transition name="otherSections">
          <div class="notDisplayed" v-if="displaySections" >
            <Section v-for="s of sections.filter((x) => x.name !== currentSection.name)" class="other-section " :name="s.name" :color="s.color" @click="changeSection(s)" :active="currentSection.name === s.name" />
          </div>
        </Transition>
      </div>
    </div>
...
.current-section {
  background-color: blue; /* New color for the current section */
}

.other-section {
  background-color: #e97878; /* Default color for other sections */
}
